...
# [... les classes PosterTextAnalyzer et LearningPosterAnalyzer restent identiques ...]
...
# Maintenant hérite de PosterTextAnalyzer
class LearningPosterAnalyzer(PosterTextAnalyzer):
    ...
    def learn_from_correction(self, original: Dict, correction: Dict):
        ...
        for field, value in correction.items():
            ...
            context = {
                ...
                # Contexte limité
                'surrounding_text': original.get('text', '')[:100]
            }
...
def run_test():
    """Exécute l'analyse sur l'exemple de test."""
    analyzer = LearningPosterAnalyzer()

    # Texte de test
    test_text = """EXPOSITION
    ...
    results = analyzer.analyze_with_learning(test_text)
    ...
    print("\nApplication d'une correction simulée :")
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyse le texte d'une affiche pour en extraire les métadonnées"
    )
    parser.add_argument(
        'filename',
        nargs='?',
        help="Nom du fichier texte à analyser"
    )
    parser.add_argument(
        '--test',
        action='store_true',
        help="Exécute l'analyse sur l'exemple de test"
    )
